...
semiannualBondIntrinsicValue <- function( semiannualInterestPayment, semiannualRateOfReturn, 
                                          faceValue, numPeriodPayments ) {
  value <- 
    (semiannualInterestPayment*(1-(1+semiannualRateOfReturn)^(-numPeriodPayments) ))/semiannualRateOfReturn 
  intrinsicValue <-
    value + faceValue/(1+semiannualRateOfReturn)^(numPeriodPayments) 
  return(intrinsicValue)
  #IF INTRINSICVALUE IS MORE THAN THE PRICE ITS SELLING ON THE MARKET BUY!!!! 
}
